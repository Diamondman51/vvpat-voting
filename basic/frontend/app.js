...
if (presedientCanidate) {
  ...
  const pVoteButtons = presedientCanidate.querySelectorAll(
    '.president button[title="vote"]'
  );
  const voteButtons = document.querySelectorAll(
    '.directors button[title="vote"]'
  );
  const directorRows = document.querySelectorAll(".directors .single");

  function checkConditions() {
    anyPresidentClicked = Array.from(pVoteButtons).some((button) =>
      button.classList.contains("active")
    );

    const allRowsHaveDisabled = Array.from(directorRows).every((row) => {
      return Array.from(row.querySelectorAll('button[title="vote"]')).some(
  ...
  // Function to log the absolute path when the page loads
  window.onload = () => {
    const absolutePath = window.location.href; // Gets the full URL of the current page
    console.log("Absolute Path:", absolutePath); // Logs the absolute path to the console
  };
  ...
  voteButtons.forEach((btn) => {
    btn.addEventListener("click", function () {
      if (!btn.classList.contains("active")) {
        btn.innerHTML =
          '<img src="../../static/images/check-mark.png" alt="Voted" style="width: 50px; height: 50px;">';
